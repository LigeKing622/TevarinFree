...
import dev.tevarin.manager.PacketManager;
import dev.tevarin.module.Category;
import dev.tevarin.module.Module;
import dev.tevarin.module.impl.misc.AntiBot;
...
import dev.tevarin.module.impl.world.Stuck;
...
import dev.tevarin.value.impl.ColorValue;
...
public class KillAura extends Module {
    ...
    public ModeValue rotMode = new ModeValue("Rotation Mode", new String[]{"Normal", "HvH", "Smart", "CNM", "Grim", "Improved"}, "HvH");
    public ModeValue aimPoint = new ModeValue("Aim Point", getAimPointNames(), "Closest", () -> rotMode.is("Improved"));

    public ModeValue abMode = new ModeValue("AutoBlock mode", new String[]{"Off", "Grim", "Key Bind", "UseItem", "Watchdog 1.9+", "Fake","Vanilla"}, "Grim");
            ...
            for (Entity ent : targets) {
                ...
                switch (markMode.getValue()) {
                    ...
                    case "Exhi":
                        ...
                        RenderUtil.drawAxisAlignedBB(offsetBB3, false, color2);
                        if (this.rangeTarget.get()) {
        ...
        if (isSword() && renderBlocking) {
            switch (abMode.getValue()) {
                ...
                case "Watchdog 1.9+":
                    ...
                    dev.tevarin.utils.PacketUtil.sendPacketNoEvent(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, EnumFacing.DOWN));
                    break;
                case "Fake":
                    break;
            }
    ...
    private void doBlock() {
        if (isSword() && !getModule(Blink.class).getState()) {
            switch (abMode.getValue()) {
                case "Vanilla": {
                    if (!isBlocking) {
                      dev.tevarin.utils.PacketUtil.sendBlocking(true, false);
                        isBlocking = true;
                    }
                    ++this.autoblockTicks;
                    break;
                ...
                case "Grim":
                    break;
                case "Watchdog 1.9+":
                    dev.tevarin.utils.PacketUtil.sendPacketNoEvent(new C08PacketPlayerBlockPlacement(new BlockPos(-1, -1, -1), 255, mc.thePlayer.inventory.getCurrentItem(), 0.0F, 0.0F, 0.0F));
